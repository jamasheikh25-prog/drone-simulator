...
void SimulationModel::scheduleTrip(const JsonObject& details) {
  ...
  CoolingStation* coolingStation = nullptr;
  ...
  if (Weather::getInstance().getTemperature() >= 60) {
  for (auto& [id, entity] : entities) {
    if (CoolingStation* cooler = dynamic_cast<CoolingStation*>(entity)) {
          coolingStation = cooler;
          break;
      }
    }
  }
  if (receiver && package && coolingStation) {
    package->initDelivery(receiver);
    std::string strategyName = details["search"];
    package->setStrategyName(strategyName);
    package->setStrategyName("cooling");
    scheduledDeliveries.push_back(package);
    controller.sendEventToView("DeliveryScheduled", details);
    Weather::getInstance().randomWeather();
    Weather::getInstance().updateTemperature();
  }
  if (receiver && package && !coolingStation) {
    package->initDelivery(receiver);
    std::string strategyName = details["search"];
